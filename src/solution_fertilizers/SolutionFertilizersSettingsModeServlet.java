...
public class SolutionFertilizersSettingsModeServlet extends HttpServlet {
    @Override
    protected void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        boolean resMode1;//Новое решение - true; Использовать существующую систему земледелия - false
        String existingResMode1;//Существующая система земледелия
        String testField = null; //поле
        String testFert; //удобрения
        boolean coeff1;
        ...
        if(landBase == null) {
            req.getRequestDispatcher("/solution_fertilizers/solution_fertilizers_single_field_settings_mode.jsp").forward(req, resp);
            return;
        }
        ...
        resMode1 = Boolean.parseBoolean(req.getParameter("agricultureSystRB"));
        System.out.println("система земледелия: " + resMode1);

        if(resMode1) {
            /*Выбор системы земледелия*/
            Collection<ResultMode1> results = dm1.getResultsAgriculture(landBase);
            if (results == null)
                return;
            existingResMode1 = req.getParameter("agricultureSystSelect");
            if (existingResMode1 == null) {
                Map<String, String> mapResults = new HashMap<String, String>();
                String text_field = rb.getString("FERT_FORM_SETT_AGR_UNKN");
                for (ResultMode1 res : results) {
                    mapResults.put(Integer.toString(res.getResultMode1Id()), text_field + " " + res.getNameResult());
                    System.out.println(Integer.toString(res.getResultMode1Id()) + ", " + text_field + " " + res.getNameResult());
                }
                String json = new Gson().toJson(mapResults);
                resp.setContentType("application/json");
                resp.setCharacterEncoding("UTF-8");
                resp.getWriter().write(json);
                return;
            }
            ResultMode1 res = dmf.getResultMode1(results, existingResMode1);
            System.out.println("Выбрана существующая система земледелия:" + res.getNameResult());
            ...
            AgricultureField field = res.getAgricultureField();
            System.out.println("Системе земледелия "+ res.getNameResult()+" соответсвует Поле #"+field.getFieldNumber());
            System.out.println("Выбрано поле:" + field.getAgricultureFieldId());
            /*Костыль, текущее поле пользователя нужно для формирования данных из csv файлов почвообследования*/
            req.getSession().setAttribute("CurrentUserField",field);

            /*По выбраному полю инициализируем тип земли и площадь*/
            TypeOfSoil typeOfSoil1 = field.getTypeOfSoil();
            float area = field.getArea();
            /*По выбраному полю инициализируем глубину пахотного слоя и объемную массу почвы*/
            float depth = field.getTopsoilDepth();
            float bulk = field.getSoilBulk();

            /*По выбраному решению инициализируем культуру, сорт, тип почвы и площадь поля*/
            GradeOfCrop gradeOfCrop = res.getGrade();
            String gradeOfCropId = Integer.toString(gradeOfCrop.getGradeOfCropId());
            String crop = gradeOfCrop.getCrop().getNameCrop()+ " " +gradeOfCrop.getNameGradeOfCrops();
            String predecess_id = String.valueOf(res.getPred().getCrop().getCropId());
            String predess = res.getPred().getCrop().getNameCrop();

            /*По выбраному решению инициализируем планируемую урожайность и систему удобрений (химическая обработка)*/
            float prod = res.getProd();
            String app_fert_id = String.valueOf(res.getFert().getApplicationOfFertilizersId());
            String app_fert = String.valueOf(res.getFert().getCodeApplicationOfFertilizers())+" "+String.valueOf(res.getFert().getDescriptionApplicationOfFertilizers());

            /*Обрабатываем выбор поля*/
            //testField = req.getParameter("field1");
            if (req.getParameter("area1") == null && req.getParameter("typeSoil1") == null && req.getParameter("planningYield") == null && req.getParameter("predSelect") == null) {
                JSONObject result = new JSONObject();
                //Врываюсь в код ЧДЕ
                /*Collection<Predecessor> r_2 = dm1.getAllPredecessors();
                if (r_2 == null)
                    return;
                Map<String, String> prev_color = new HashMap<String, String>();
                Map<String, String> prev_pattern = new HashMap<String, String>();
                for(Predecessor r : r_2) {
                    if(r.getPredecessorId() == 1) {
                        String text_pred = rb.getString("PL_FORM_SETT_PRED_UNKN");
                        prev_color.put(Integer.toString(r.getPredecessorId()), text_pred);
                        prev_pattern.put(Integer.toString(r.getPredecessorId()), text_pred);
                        continue;
                    }
                    prev_color.put(r.getCrop().getCodeCrop(), r.getCrop().getColor());
                    prev_pattern.put(r.getCrop().getCodeCrop(), r.getCrop().getPattern());
                }
                result.put("prev_color", prev_color);
                result.put("prev_pattern", prev_pattern);*/


                String text_field = rb.getString("FERT_FORM_SETT_FIELD_NUMBER");
                Map<String, String> mapFields = new HashMap<String, String>();
                mapFields.put(Integer.toString(field.getAgricultureFieldId()), text_field+Integer.toString(field.getFieldNumber()));
                result.put("field1", mapFields);
                result.put("field_number",field.getFieldNumber());
                ...
                mapGrades.put(gradeOfCropId, crop);
                result.put("corpSelect", mapGrades);

                Map<String,String>mapFert = new HashMap<>();
                mapFert.put(app_fert_id, app_fert);
                result.put("fertSelect", mapFert);

                result.put("planningYield", BigDecimal.valueOf(prod).setScale(2,BigDecimal.ROUND_HALF_UP));

                Map<String, String> mapPredess = new HashMap<String, String>();
                mapPredess.put(predecess_id, predess);
                result.put("predSelect", mapPredess);
            ...
            System.out.println("Культура "+ gradeOfCrop.getCrop().getNameCrop() +" сорт "+ gradeOfCrop.getNameGradeOfCrops());
            System.out.println("Планируемая урожайность "+prod);
            System.out.println("Химическая обработка "+app_fert);
            ...
            /*int NITROGEN_ID = microelements.get(FertilizerCalculations.MICROELEMENT_NITROGEN).getMicroelementId();
            int PHOSPHORUS_ID = microelements.get(FertilizerCalculations.MICROELEMENT_PHOSPHORUS).getMicroelementId();
            int POTASSIUM_ID = microelements.get(FertilizerCalculations.MICROELEMENT_POTASSIUM).getMicroelementId();*/
            ...
            if (req.getParameter("coeff1")!=null||req.getParameter("coeff2")!=null||req.getParameter("coeff3")!=null){
                JSONObject result = getCoefficientJSON(req,calculations,res.getGrade().getCrop(),isN,isPh,isP);
                resp.setContentType("application/json");
                resp.setHeader("Access-Control-Allow-Origin", "*");
                resp.getWriter().write(result.toString());
                return;
            ...
            if (req.getParameter("calc")!=null){
                JSONObject result = getCalculationsResult(req, resp, calculations, dmf, date, newNameSolution, res, isN, isPh, isP, gradeOfCrop.getCrop(), typeOfSoil1, field, res.getPred(), res.getFert(), prod);
                resp.setContentType("application/json");
                resp.setHeader("Access-Control-Allow-Origin", "*");
                resp.getWriter().write(result.toString());
                return;
        ...
        /*Получаем все поля активной земельной базы хозяйства */
            Collection<AgricultureField> fields = dmf.getAllFields(landBase);
            if (fields == null)
                return;
        /*Обрабатываем выбор поля*/
            testField = req.getParameter("field1");
            ...
            if (req.getParameter("coeff1")!=null||req.getParameter("coeff2")!=null||req.getParameter("coeff3")!=null){
                JSONObject result = getCoefficientJSON(req,calculations,cropForCalculations,isN,isPh,isP);
                resp.setContentType("application/json");
                resp.setHeader("Access-Control-Allow-Origin", "*");
                resp.getWriter().write(result.toString());
                return;
            ...
            /*TODO:РАССЧЕТ УДОБРЕНИЙ*/
            if (req.getParameter("calc")!=null){
                JSONObject result = getCalculationsResult(req, resp, calculations, dmf, date, newNameSolution, res, isN, isPh, isP, cropForCalculations, typeOfSoil1, field, pred, application, Float.parseFloat(testPlanningYield));
                resp.setContentType("application/json");
                resp.setHeader("Access-Control-Allow-Origin", "*");
                resp.getWriter().write(result.toString());
                return;
    ...
    /*ВОЗВРАЩАЕТ JSON С КОЭФФИЦИЕНТАМИ ДЛЯ 1 ИЗ 3 ПАНЕЛЕЙ*/
    private JSONObject getCoefficientJSON(HttpServletRequest req, FertilizerCalculations calculations,
                                          Crop cropForCalculations, boolean isN, boolean isPh, boolean isP){
        Locale l = new Locale("ru");
        ResourceBundle rb = ResourceBundle.getBundle("Resource", l);
        ...
        if(req.getParameter("coeff1")!=null) {
            boolean coeff1 = Boolean.parseBoolean(req.getParameter("coeff1"));
            /*TODO:ПРОВЕРИТЬ ФОРМИРОВАНИЕ ДАННЫХ ИЗ БД*/
            /*проверка того, удобрения с какими микроэлементами участвуют в хим.обработке*/
            if(!isN)
                result.put("nitrogen1", rb.getString("FERT_FORM_NO_MICROELEMENT_IN_FERT"));
            else if (isN&&!coeff1){
                float nitrogen1 = calculations.getRemovalForCropAndMicroelement(cropForCalculations, microelements.get(FertilizerCalculations.MICROELEMENT_NITROGEN));
                result.put("nitrogen1", BigDecimal.valueOf(nitrogen1).setScale(2,BigDecimal.ROUND_HALF_UP));
            }

            if(!isPh)
                result.put("phosphorus1", rb.getString("FERT_FORM_NO_MICROELEMENT_IN_FERT"));
            else if (isPh&&!coeff1){
                float phosphorus1 = calculations.getRemovalForCropAndMicroelement(cropForCalculations,microelements.get(FertilizerCalculations.MICROELEMENT_PHOSPHORUS));
                result.put("phosphorus1", BigDecimal.valueOf(phosphorus1).setScale(2,BigDecimal.ROUND_HALF_UP));
            }

            if(!isP)
                result.put("potassium1", rb.getString("FERT_FORM_NO_MICROELEMENT_IN_FERT"));
            else if (isP&&!coeff1){
                float potassium1 = calculations.getRemovalForCropAndMicroelement(cropForCalculations,microelements.get(FertilizerCalculations.MICROELEMENT_POTASSIUM));
                result.put("potassium1", BigDecimal.valueOf(potassium1).setScale(2,BigDecimal.ROUND_HALF_UP));
            }
        }
        ...
        if(req.getParameter("coeff2")!=null) {
            ...
            if (coeff2||req.getParameter("yearOfUsing")!=null){
                ...
                if(!isN)
                    ...
                    float nitrogen2 = calculations.getAverageDigestionCoefficient(microelements.get(FertilizerCalculations.MICROELEMENT_NITROGEN),req.getParameter("yearOfUsing").toString());
                    result.put("nitrogen2", BigDecimal.valueOf(nitrogen2).setScale(2,BigDecimal.ROUND_HALF_UP));
                }

                if(!isPh)
                    result.put("phosphorus2", rb.getString("FERT_FORM_NO_MICROELEMENT_IN_FERT"));
                else if (isPh&&!coeff2){
                    float phosphorus2 = calculations.getAverageDigestionCoefficient(microelements.get(FertilizerCalculations.MICROELEMENT_PHOSPHORUS), req.getParameter("yearOfUsing").toString());
                    result.put("phosphorus2", BigDecimal.valueOf(phosphorus2).setScale(2,BigDecimal.ROUND_HALF_UP));
                }

                if(!isP)
                    result.put("potassium2", rb.getString("FERT_FORM_NO_MICROELEMENT_IN_FERT"));
                else if (isP&&!coeff2){
                    float potassium2 = calculations.getAverageDigestionCoefficient(microelements.get(FertilizerCalculations.MICROELEMENT_POTASSIUM), req.getParameter("yearOfUsing").toString());
                    result.put("potassium2", BigDecimal.valueOf(potassium2).setScale(2,BigDecimal.ROUND_HALF_UP));
                }
            }
        }
        ...
        if(req.getParameter("coeff3")!=null) {
            boolean coeff3 = Boolean.parseBoolean(req.getParameter("coeff3"));
            /*TODO:КАКИЕ ДАННЫЕ ИЗ БД БРАТЬ?*/
            if(!isN)
    ...
    /*ВОЗВРАЩАЕТ JSON С РЕЗУЛЬТАТАМИ РАСЧЕТОВ*/
    private JSONObject getCalculationsResult(HttpServletRequest req, HttpServletResponse resp, FertilizerCalculations calculations, DataManagerModeFertilizers dmf,
                                             String date, String nameNewSolution, ResultMode1 res,
                                             boolean isN, boolean isPh, boolean isP,
                                             Crop crop, TypeOfSoil typeOfSoil, AgricultureField field, Predecessor pred, ApplicationOfFertilizers appl, float prod) throws UnsupportedEncodingException {
        Locale l = new Locale("ru");
        ResourceBundle rb = ResourceBundle.getBundle("Resource", l);
        ...
        try {
            ...
            float planningYield = Float.parseFloat(req.getParameter("planningYield"));
            ...
            float takeawayP = 0;
            if(isN)
                takeawayN = calculations.calcutationsTakeaway(prod, crop, microelement_nitrogen);
            if(isPh)
                takeawayPh = calculations.calcutationsTakeaway(prod, crop, microelement_phosphorus);
            if(isP)
                takeawayP = calculations.calcutationsTakeaway(prod, crop, microelement_potassium);
            float m = calculations.calculationsM(field.getArea()*10000, field.getTopsoilDepth(), field.getSoilBulk());//масса пахотного слоя
            float phContent = 0;
            float pContent = 0;
            float value_humus = 0;
            ...
            if(isPh) {
                ...
                number_digestible_Ph = calculations.startCalculationsAmountOfDigestibleMicroelements(microelement_phosphorus, field, crop, typeOfSoil, phosphorus, microelements_value.get("phosphorus")[2]);
                real_possible_harvest_Ph = calculations.startCalculationsRealPossibleHarvest(number_digestible_Ph, crop, microelement_phosphorus);
                ...
                need_in_Ph = calculations.startCalculationsFertilizerDosesForYield(microelement_phosphorus, field, crop, typeOfSoil, planningYield, phosphorus, predDoze, predYield, microelements_value.get("phosphorus")[2], predYear);
            }
            if(isP) {
                pContent = calculations.microelementContent(m, potassium);
                number_digestible_P = calculations.startCalculationsAmountOfDigestibleMicroelements(microelement_potassium, field, crop, typeOfSoil, potassium, microelements_value.get("potassium")[2]);
                real_possible_harvest_P = calculations.startCalculationsRealPossibleHarvest(number_digestible_P, crop, microelement_potassium);
                ...
                need_in_P = calculations.startCalculationsFertilizerDosesForYield(microelement_potassium, field, crop, typeOfSoil, planningYield, potassium, predDoze, predYield, microelements_value.get("potassium")[2], predYear);
            }
            if(isN){
                value_humus = calculations.valueHumus(humus, m);
                value_of_nitrogen_in_humus = calculations.valueOfNitrogenInHumus(value_humus);
                value_forms_of_nitrogen = calculations.valueFormsOfNitrogen(value_of_nitrogen_in_humus, crop, typeOfSoil);
                number_digestible_N = calculations.startCalculationsAmountOfDigestibleMicroelements(microelement_nitrogen, field, crop, typeOfSoil, humus, microelements_value.get("nitrogen")[2]);
                real_possible_harvest_N = calculations.startCalculationsRealPossibleHarvest(number_digestible_N, crop, microelement_nitrogen);
                ...
                need_in_N = calculations.startCalculationsFertilizerDosesForYield(microelement_nitrogen, field, crop, typeOfSoil, planningYield, humus, predDoze, predYield, microelements_value.get("nitrogen")[2], predYear);
            }
            ...
            resultsFertilizers.setResultsFertilizersName(nameNewSolution);
            if(res != null)
                resultsFertilizers.setResultMode1(res);
            resultsFertilizers.setAgricultureField(field);
            resultsFertilizers.setCrop(crop);
            resultsFertilizers.setPalnningYield(prod);
            ...
            resultsFertilizers.setSystFert(appl);
            ...
            resultsFertilizers.setAgrSyst(true);
